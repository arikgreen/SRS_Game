...
namespace Iml.Documentation.VML
{
  ...
  public partial class Path
  {
    ...
    public static Path Parse (string str)
    {
      ...
      if (TryParse(str, out result))
        return result;
      throw new FormatException(String.Format("Cannot parse Path from string \"{0}\"", str));
    }
    ...
    /// <returns></returns>
    public static bool TryParse (string str, out Path value)
    {
      return FormatHelper.TryParse<Path>(str, out value);
    }
  ...
  /// </summary>
  public partial class PathTypeConverter : TypeConverter
  {
    /// <summary>
    ...
    public override object ConvertFrom (ITypeDescriptorContext context, CultureInfo culture, object value)
    {
      if (value is string)
      {
        Path result = Path.Parse(value as string);
        return result;
      }
      else
    ...
    {
      if (destinationType == typeof(string) && value is Path)
      {
        Path val = (Path)value;
        return val.ToString();
      }
